...
def get_dashboard_data():
    ...
    return {
        ...
        'sched_df': pd.DataFrame(fm.get_schedules() if fm.get_schedules() else [])
    }
...
# --- [신규 추가] ADS Monitor 탭 (tbs[3]) ---
with tbs[3]:
    st.markdown("### 🗺️ ADS Monitor Map")
    st.link_button("↗️ 새 창에서 ADS Monitor 열기", "https://adtc.swm.ai/adsmonitor/#/map")
    components.iframe("https://adtc.swm.ai/adsmonitor/#/map", height=800, scrolling=True)

# --- 시스템 관리 탭 (권한이 있을 경우 tbs[4]로 설정) ---
if st.session_state.user_role in ['admin', 'DM']:
    with tbs[4]:
